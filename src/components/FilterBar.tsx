  ...
  return (
    <div className="flex items-center gap-4 px-5 py-3 flex-shrink-0"
      ...
      <button type="button" onClick={openSidebar} aria-label="Open sidebar"
              className="lg:hidden w-9 h-9 flex items-center justify-center rounded-[8px]"
              style={{ border: '1px solid var(--line)', color: 'var(--ink-3)' }}>
        <Menu size={17} />
      </button>

      <div className="flex-1 min-w-0">
        <h1 className="font-extrabold text-[20px] tracking-tight leading-none" style={{ color: 'var(--ink)' }}>
          {title}
        </h1>
        {subtitle && (
      ...
      <button type="button" onClick={onRefresh} disabled={loading} aria-label="Refresh"
              className="w-9 h-9 flex items-center justify-center rounded-[8px] disabled:opacity-40"
